...
ventana = Tk()
ventana.title("Crud MySql with Tkinter")
...
btnModificar=Button(marco, text="Modificar", command=lambda:actualizar())
...
def validarCampos():
    return len(patente.get()) and len(hora_ingreso.get()) and len(e_disponible.get()) and len(e_disponible.get()) and len(c_auto.get()) and len(c_tipo_a.get()) and len() and len() and len(c_mensual.get()) and len(c_anual.get()) and len(funcionario.get())
...
def actualizar():
    if modificar == True:
        if validarCampos():
         val=(patente.get(),hora_ingreso.get(),e_disponible.get(),c_auto.get(),c_tipo_a.get(),c_mensual.get(),c_anual.get(),funcionario.get())
         sql="update ingreso_vehiculos set patente=%s, hora_ingreso=%s, e_disponible=%s, c_auto=%s, c_tipo_a=%s, c_mensual=%s, c_anual=%s, funcionario=%s"
         db.cursor.execute(sql, val)
         db.connection.commit()
